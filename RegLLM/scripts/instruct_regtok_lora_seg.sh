#!/bin/bash
# LoRA instruction tuning for MedSIGHT (Qwen3-8B + RegTok + segmentation head).
#
# Lightweight alternative to the full-FT script — trains a LoRA adapter on top
# of the alignment checkpoint instead of updating all LLM weights. Useful for
# adapting MedSIGHT to a downstream corpus with limited compute.
#
...
SFT_OUT_PATH="${CKPT_ROOT}/medsight_lora64_instruct"
...
deepspeed llava/train/train_mem.py \
    ...
    --data_path "${DATA_ROOT}/RegAlign/All_mixture.json" \
