...
def evaluate_mc(ground_truth_data, test_data):
    ...
    for gt_sample, test_sample in zip(ground_truth_data, test_data):
        ...
        if "gt_answer" in gt_sample:
            label = gt_sample['gt_answer']
        else:
...
def evaluate(gt, pred, candidate=None, criterion=None):    
    ...
    for gt_item, pred_item in zip(gt, pred):
        gt_value = gt_item['gt_answer'].lower()
...
if __name__ == '__main__':
    ...
    if not args.report:
        ...
        # if num_gt_ids > num_pred_ids:
        #     gt_ids = gt_ids[:num_pred_ids]
        ...
        assert len(gt_ids) == len(pred_ids), "please make sure pred and gt are exactly matched"
    if args.mc:
        # perform evaluation
