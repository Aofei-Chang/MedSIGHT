...
from .criterion import hungarian_assign, compute_losses_with_indices, hungarian_assign_with_semantic
...
class PerceiverCrossAttention(nn.Module):
    ...
    def forward(self, x, region_latents, pos=None, query_pos=None, attn_mask=None):
        ...
        """
        # print(x.dtype, self.norm_media.weight.dtype, "dtype of norm")
...
class SegmentationHead(nn.Module):
    ...
    """
    def __init__(self, dim, num_classes=None, num_heads=None, interpolate_scale_factor=4):
        super().__init__()
    ...
    def forward(self, region_tokens, image_features, output_mask=True, use_interpolate=True):
        ...
        # Compute similarity
        # print(region_tokens_proj.dtype, image_features_flat.dtype, "dtype of region_tokens_proj and image_features_flat")
        ...
        if output_mask and use_interpolate:
            # Use scale_factor directly for F.interpolate
            interp_mask_logits = F.interpolate(mask_logits, scale_factor=scale_factor, mode="bilinear", align_corners=False)
            new_h, new_w = interp_mask_logits.shape[-2:]
            interp_mask_flat = interp_mask_logits.view(b, n, new_h * new_w)
            interp_attn_mask = interp_mask_flat.unsqueeze(1).unsqueeze(2)  # (b, 1, 1, n, new_h*new_w)
            interp_attn_mask = interp_attn_mask.repeat(1, self.num_heads, 1, 1, 1)
...
class ConvAdapter(nn.Module):
    """
    Lightweight upsampling adapter: smaller scale upsample + 3x3 conv.
    """
...
class LightweightRegionDecoder(nn.Module):
    ...
    def forward(self, codes, image_features, multi_scale_image_features=None, interp_attn_mask=None):
        ...
        for i, (attn, norm, ff, ff_norm) in enumerate(zip(self.cross_attn_blocks, self.norms, self.ff_layers, self.ff_norms)):
            ...
            # print(prev_attn_mask.shape if prev_attn_mask is not None else None, "prev_attn_mask shape in LightweightRegionDecoder")
...
class RegionPerceiver(nn.Module):
    ...
    """
    def __init__(self, dim=512, num_queries=16, num_stacks=1, num_stages=2, interpolate_scale_factor=2, dim_head=64, heads=8, ff_mult=4, num_classes=None, do_quantize=False, 
                 quantizer=None, quantize_intermediate=False, finetune_codebook_only=False, upsample_mode: str = "conv",
                 use_self_attn = False, semantic_label_dim=None):
        super().__init__()
        ...
        if self.upsample_mode == "query":
            ...
            self.conv_adapters = nn.ModuleList([ConvAdapter(dim, scale_factor=self.interpolate_scale_factor) for _ in range(num_stages)])
        ...
        self.segmentation_cls_head = SegmentationHead(dim=dim, num_classes=num_classes, num_heads=heads, interpolate_scale_factor=interpolate_scale_factor)
        ...
        if self.do_quantize:
            ...
            self.code_decoder = LightweightRegionDecoder(
                dim=dim, code_dim=self.code_dim, num_layers=3, use_multiscale=False,
                num_classes=num_classes, num_heads=heads
            )
        # Add a projection layer for semantic alignment if needed
        self.semantic_label_dim = semantic_label_dim
        if semantic_label_dim is not None and semantic_label_dim != dim:
            self.semantic_proj = nn.Linear(dim, semantic_label_dim, bias=False)
        else:
    ...
    def forward(self, image_features, mask_labels=None, class_labels=None, loss_type="dice_bce", mask=None, do_quantize=False, deep_supervision=False, eos_coef=0.1,
        ...
        self.dtype = self.region_queries.dtype
        # print(self.region_queries.dtype, "dtype of region_queries", image_features.dtype)
        if image_features.dtype != self.region_queries.dtype:
            image_features = image_features.to(self.region_queries.dtype)
        ...
        for stage in range(self.num_stages):
            ...
            current_image_features = current_image_features.to(self.dtype)
            current_region_queries = current_region_queries.to(self.dtype)
            if self.upsample_mode == "query":
            ...
            input_region_queries = current_region_queries.to(self.dtype)
            input_image_features = upsampled_image_features.to(self.dtype)
            ...
            current_region_queries = input_region_queries.to(self.dtype)
            current_image_features = input_image_features.to(self.dtype)
        ...
        indices_normal = None
        semantic_loss = None
        semantic_loss_normal, semantic_loss_quant = None, None
        if mask_labels is not None:
            # Use semantic-aware Hungarian if semantic_labels are provided
            if semantic_labels is not None:
                # Project region queries to semantic label dim if needed
                region_queries_for_semantic = self.semantic_proj(current_region_queries)
                indices_normal, dice_loss_normal, bce_loss_normal, cls_loss_normal, semantic_loss_normal = hungarian_assign_with_semantic(
                    seg_logits_normal, mask_labels, class_logits_normal, class_labels, self.num_classes, eos_coef, loss_type,
                    current_region_queries=region_queries_for_semantic, semantic_labels=semantic_labels, semantic_temperature=semantic_temperature
                )
            else:
            ...
            if seg_logits_quant is not None:
                if semantic_labels is not None:
                    region_queries_for_semantic = self.semantic_proj(current_region_queries)
                    indices, dice_loss_quant, bce_loss_quant, cls_loss_quant, semantic_loss_quant = hungarian_assign_with_semantic(
                ...
                cls_loss = cls_loss_quant
                semantic_loss = semantic_loss_quant
            else:
                dice_loss = dice_loss_normal
                bce_loss = bce_loss_normal
                cls_loss = cls_loss_normal
                semantic_loss = semantic_loss_normal
