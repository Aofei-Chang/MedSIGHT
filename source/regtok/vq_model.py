...
class ModelArgs:
    ...
    # Index into the UniMed-CLIP transformer's `image_intermediates` list (0..L-1 or
    # negative indices). -1 = final layer, -2 = penultimate layer, etc. This is the
    # vision feature layer fed into the region perceiver, mirroring LLaVA's
    # `mm_vision_select_layer` argument.
    select_layer: int = -1
...
class RegTok(nn.Module):
    ...
    def forward(self, input, mask_labels=None, class_labels=None, do_quantize=False, loss_type="dice_bce", semantic_labels=None, llm_training=False, modality_label=None):
        ...
        with torch.no_grad():
            ...
            # Honour the configured layer index instead of always taking the final
            # transformer block — LLaVA-style `mm_vision_select_layer = -2` keeps
            # pre-LN features, which is what the alignment / instruction scripts use.
            feats = vision_output['image_intermediates'][self.select_layer]
            # print(feats.shape, "feats shape", self.select_layer, "select layer")
            cls_token = feats[:, 0, :].detach().clone()          # (B, D)
            patch_features = feats[:, 1:, :].detach().clone()     # (B, L-1, D)
